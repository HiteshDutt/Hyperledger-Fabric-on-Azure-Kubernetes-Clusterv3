...
createOrgMSP() {
    ...
    rm -rf ${CRYPTO_CONFIG}/${nodeType}/${orgName}/*
    mkdir -p ${CRYPTO_CONFIG}/${nodeType}/${orgName}/msp/{admincerts,cacerts,keystore,signcerts,tlscacerts}
    mkdir -p ${CRYPTO_CONFIG}/${nodeType}/${orgName}/users/admin.${orgName}/msp/{admincerts,cacerts,keystore,signcerts,tlscacerts}
    mkdir -p ${CRYPTO_CONFIG}/${nodeType}/${orgName}/tls
    mkdir -p ${CRYPTO_CONFIG}/${nodeType}/${orgName}/ca/tlsca
    #admincerts
    cat ${adminProfilePath} | jq '.cert' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/msp/admincerts/cert.pem
    cat ${adminProfilePath} | jq '.cert' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/users/admin.${orgName}/msp/admincerts/cert.pem

    #cacerts
    cat ${mspProfilePath} | jq '.cacerts' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/msp/cacerts/rca.pem
    cat ${mspProfilePath} | jq '.cacerts' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/users/admin.${orgName}/msp/cacerts/rca.pem


    cat $connectionProfilePath | jq '.certificateAuthorities.'${orgName}'CA.tlsCACerts.pem' | tr -d '"' | sed 's/\\n/\n/g' > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/ca/tlsca/cert.pem
   ...
   for row in  $(cat $connectionProfilePath | jq '.organizations.'${orgName}'.'$nodeType's[]'); do
    ...
    checkNodeName ${nodeType} ${connectionProfilePath} ${row}
     mkdir -p ${CRYPTO_CONFIG}/${nodeType}/${orgName}/${folderName}/msp/tlscacerts
    #tlscacerts
    cat ${connectionProfilePath} | jq '.'$nodeType's.'$row'.tlsCACerts.pem' | tr -d '"' | sed 's/\\n/\n/g' > \
    ${CRYPTO_CONFIG}/${nodeType}/${orgName}/$folderName/msp/tlscacerts/ca.crt

   done
   echo "path nodeType/orgName "${nodeType}"/"${orgName}
    #signcerts
    cp ${CRYPTO_CONFIG}/${nodeType}/${orgName}/msp/admincerts/cert.pem ${CRYPTO_CONFIG}/${nodeType}/${orgName}/msp/signcerts/cert.pem
    cp ${CRYPTO_CONFIG}/${nodeType}/${orgName}/msp/admincerts/cert.pem ${CRYPTO_CONFIG}/${nodeType}/${orgName}/users/admin.${orgName}/msp/signcerts/admin.${orgName}@${orgName}-cert.pem


    #keystore
    cat ${adminProfilePath} | jq '.private_key' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/msp/keystore/key.pem
    cat ${adminProfilePath} | jq '.private_key' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/users/admin.${orgName}/msp/keystore/priv_sk


    #admin-tls-cert
    cat ${adminProfilePath} | jq '.tls_cert' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/tls/cert.pem

    #admin-tls-key
    cat ${adminProfilePath} | jq '.tls_private_key' | tr -d '"' | base64 -d > ${CRYPTO_CONFIG}/${nodeType}/${orgName}/tls/key.pem
}
